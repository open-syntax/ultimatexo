...
export async function onRequest(context) {
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          error: "Bad Gateway",
          message:
            "Cloudflare Pages middleware failed to reach backend server.",
          details: String(err),
        }),
        {
          status: 502,
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          error: "Bad Gateway",
          message:
            "Cloudflare Pages middleware failed to establish WebSocket connection to backend.",
          details: String(err),
        }),
        {
          status: 502,
  ...
  // Pass non-API, non-WS requests to static page router
  return next();
}
